...
void fill_guild_data(guildbook_data &guildbook, std::string guild_name,
                     const sol::table &guild_data) {
...
auto guildbook_data::parse(std::string_view lua) -> guildbook_data {
  ...
  lua_state.set_exception_handler(
      [](lua_State *L, sol::optional<const std::exception &> e,
         sol::string_view str) -> int { throw *e; });
  env["_G"] = env;
  ...
  if (characters.has_value()) {
    for (const auto &[key, value] : *characters) {
      if (key.is<std::string>() && value.is<sol::table>()) {
        ret.add_character(character::from_guildbook(key.as<std::string>(),
                                                    value.as<sol::table>()));
      }
    }
  }
...
void guildbook_data::add_guild(const guild &g) {
  auto where =
      std::ranges::lower_bound(_guilds, g.name(), std::less{}, &guild::name);

  if (where->name() == g.name()) {
    *where = g;
  } else {
...
void guildbook_data::add_guild(guild &&g) {
  ...
  if (where != _guilds.end() && where->name() == g.name()) {
    *where = std::move(g);
  } else {
...
void guildbook_data::add_character(const character &g) {
  auto where = std::ranges::lower_bound(_characters, g.name, std::less{},
                                        &character::name);
