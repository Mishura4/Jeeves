...
auto character::from_guildbook(std::string name, const sol::table &data) -> character {
	...
	if (sol::optional<int> v = data["fishingLevel"]; v.has_value())
		ret.fishing_level = *v;
	if (sol::optional<sol::table> v = data["profession1Recipes"]; v.has_value() && !v->empty()) {
	...
	if (sol::optional<sol::table> v = data["profession2Recipes"]; v.has_value() && !v->empty()) {
